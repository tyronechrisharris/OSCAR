...
public class OshExtension implements ExtensionMain, EmbeddedExtension, IMqttServer
{
    ...
    // Map to hold temporary tokens generated by the frontend for proxy authentication
    // Cache expires tokens after 10 seconds to prevent memory leaks if clients drop before connecting
    static com.google.common.cache.Cache<String, String> proxyTokens = com.google.common.cache.CacheBuilder.newBuilder()
            .expireAfterWrite(10, java.util.concurrent.TimeUnit.SECONDS)
            .build();

    public static void registerProxyToken(String token, String user) {
        proxyTokens.put(token, user);
    }

    public static String consumeProxyToken(String token) {
        String user = proxyTokens.getIfPresent(token);
        if (user != null) {
            proxyTokens.invalidate(token);
        }
        return user;
    }
