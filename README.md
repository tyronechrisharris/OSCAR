...
For production use, **Ubuntu Server 24.04.4 LTS** is the preferred and recommended operating system. Docker on Linux provides the best security, performance, and automated orchestration for the OSCAR stack. Due to host-level network limitations on Windows, the Windows deployment script (`launch-all.bat`) automatically downloads and manages a native `mediamtx.exe` host process to bypass Docker for Windows networking constraints, allowing automated MediaMTX path provisioning to function identically to Linux.
