...
- **Bifurcated Authentication (TOTP + API Keys)**: OSCAR implements two distinct authentication flows to ensure secure human access and robust machine-to-machine (M2M) communication.
  ...
  - **MQTT/WebSocket Authentication**: Secured via a short-lived **MQTT Ticket System**. To establish a telemetry connection, clients must first fetch a signed ticket from the backend REST API (`/api/mqtt/ticket`). These tickets are HMAC-SHA256 signed using the system's root secrets, preventing unauthorized connection attempts to the message broker.
...
- **MQTT Ticket Lifecycle**: Telemetry connections are secured via a short-lived **HMAC-SHA256 MQTT Ticket System**. To establish a connection, clients must first fetch a signed ticket from the backend REST API (`/api/mqtt-ticket`).
