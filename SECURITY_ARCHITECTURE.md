...
- **MediaMTX Host Network Proxy**: To provide stable, low-latency RTSP streaming and avoid docker bridge network bottlenecks, the `mediamtx` sidecar operates with `network_mode: "host"`. The OSH backend internally commands this proxy by sending automated HTTP REST payload commands to the Docker bridge gateway (e.g. `172.17.0.1:9997`, configurable via `MEDIAMTX_IP`). These API requests are secured via HTTP Basic Authentication using `MEDIAMTX_API_USER` and `MEDIAMTX_API_PASS`. To prevent Tailscale's global SOCKS5 proxy from intercepting and blocking these local API requests, `LaneSystem` explicitly configures its `HttpClient` with `NO_PROXY`.
