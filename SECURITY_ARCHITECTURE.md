...
- **Local Sensor TCP Sockets**: Local physical sensors (e.g., Rapiscan and Aspect RPMs) connect via raw TCP sockets (`TCPCommProvider`). To prevent the global `-DsocksProxyHost` JVM argument from routing these local LAN IPs into the Tailnet (causing `SOCKS server general failure`), the `docker-compose.yml` whitelists local subnets using the `socksNonProxyHosts` parameter (configurable via the `NON_PROXY_HOSTS` environment variable) and `TCPCommProvider` establishes sockets using `Proxy.NO_PROXY`.
...
- **Bifurcated Authentication (TOTP + API Keys)**: OSCAR implements two distinct authentication flows to ensure secure human access and robust machine-to-machine (M2M) communication.
  ...
  - **Machine/API Routes (SOS, SPS)**: Secured via long-lived API Keys. Keys are provided via `Authorization: Bearer <key>` or `X-API-Key` headers. These routes return standard `401/403` status codes instead of redirects if authentication fails.
  - **MQTT/WebSocket Authentication**: Secured via a short-lived **MQTT Ticket System**. To establish a telemetry connection, clients must first fetch a signed ticket from the backend REST API at the API-root-relative path `/mqtt-ticket` (e.g., `/sensorhub/api/mqtt-ticket`). These tickets are HMAC-SHA256 signed using the system's root secrets, preventing unauthorized connection attempts to the message broker.
...
- **MQTT Ticket Lifecycle**: Telemetry connections are secured via a short-lived **HMAC-SHA256 MQTT Ticket System**. To establish a connection, clients must first fetch a signed ticket from the backend REST API path `/mqtt-ticket`.
  - **Authentication**: These tickets are signed using the system's root secrets (`javax.net.ssl.keyStorePassword`), preventing unauthorized connection attempts to the message broker.
  - **Validation**: Validation occurs once at the MQTT CONNECT/authentication level for the shared connector, rather than per-message, ensuring high performance at scale (e.g., 50+ lanes).
  - **Refresh**: Tickets include a `refreshAfterSeconds` claim (defaulting to 12 minutes for a 15-minute TTL). The frontend automatically initiates a proactive refresh of these credentials and updates the shared connector state to ensure uninterrupted telemetry streams.
  - **Fail-Secure**: If a backend secret is not configured or the ticket fetch fails, the ticket system fails closed, and the frontend refuses to initialize MQTT-based datasources, returning a 401 Unauthorized error.
