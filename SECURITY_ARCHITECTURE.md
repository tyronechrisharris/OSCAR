...
- **Docker Secrets**: Passwords (`.db_password`, `.app_secrets`) are dynamically generated by an `init-secrets` container before any other service starts. They are persisted to an `oscar_secrets` volume and mounted securely to the database and backend.
