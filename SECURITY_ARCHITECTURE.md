...
- **MQTT Ticket Lifecycle**: Tickets generated by the backend include a `refreshAfterSeconds` claim. The frontend automatically initiates a proactive refresh of these credentials before they expire to ensure uninterrupted telemetry streams. If a backend secret is not configured, the ticket system fails closed, returning a 401 Unauthorized error to any fetch attempts.
