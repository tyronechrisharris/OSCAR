...
The system uses a Hybrid Volume Architecture (Named Volumes) to manage database and application passwords.
- **Docker Secrets**: Passwords (`.db_password`, `.app_secrets`) are dynamically generated by an `init-secrets` container before any other service starts. They are persisted to a highly secure `oscar_secrets` named volume and mounted securely to the database and backend (at `/secrets`).
