...
- **MediaMTX Host Network Proxy**: To provide stable, low-latency RTSP streaming and avoid docker bridge network bottlenecks, the `mediamtx` sidecar operates with `network_mode: "host"`. The OSH backend internally commands this proxy by sending automated HTTP REST payload commands to the Docker bridge gateway (e.g. `172.17.0.1:9997`, configurable via `MEDIAMTX_IP`). These API requests are secured via HTTP Basic Authentication. To enforce the **Principle of Least Privilege**, a dedicated `mtx_secrets` Docker volume is used to isolate MediaMTX configuration from high-sensitivity system secrets. A 16-character random hex password (`.mediamtx_api_pass`) and default username (`.mediamtx_api_user`) are automatically generated by the `init-secrets` container and baked into a runtime-generated `mediamtx.yml` file. The OSH backend reads these credentials via `_FILE` environment variables to establish the authenticated control link. To prevent Tailscale's global SOCKS5 proxy from intercepting and blocking these local API requests, `LaneSystem` explicitly configures its `HttpClient` with `NO_PROXY`.
