...
- **MQTT Connectivity**: The system maintains stable MQTT connections over WebSockets. Authentication is managed via a dedicated ticket-granting endpoint (`/api/mqtt/ticket`) which provides short-lived credentials. Both the OSH backend and the reverse proxy are configured with an increased 10-minute idle timeout to prevent frequent disconnections in high-latency environments (e.g., Tailscale mesh). Additionally, the frontend MQTT client implements a proactive ticket refresh mechanism and a 15-second keepalive interval to ensure the connection remains active and authorized. The frontend `Node.ts` natively abstracts the WebSocket secure URL path generation, automatically replacing internal server ports with external routing hostnames (via Magic DNS `.ts.net` routing) when required by the deployed environment configuration.
