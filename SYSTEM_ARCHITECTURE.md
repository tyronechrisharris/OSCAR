...
- **MQTT Connectivity**: The system maintains stable MQTT connections over WebSockets. Authentication is managed via a dedicated centralized ticket-granting endpoint (`/api/mqtt-ticket`) which provides HMAC-SHA256 signed credentials for the `__mqtt_ticket__` user. This architecture replaces the legacy per-datasource proxy token model, significantly reducing authentication overhead.
  - **Routing**: MQTT WebSocket traffic is routed through the same external public URL (e.g., Tailscale MagicDNS or LAN HTTPS) as the REST API, typically on path `/sensorhub/mqtt`.
  - **Persistence**: Both the OSH backend and the reverse proxy are configured with an increased 10-minute idle timeout to prevent frequent disconnections in high-latency environments (e.g., Tailscale mesh).
  - **Lifecycle**: The frontend MQTT client implements a proactive ticket refresh mechanism (based on the `refreshAfterSeconds` claim) and a 15-second keepalive interval to ensure the connection remains active and authorized without re-authenticating every datastream.
  - **Abstraction**: The frontend `Node.ts` natively abstracts the WebSocket secure URL path generation, automatically stripping internal backend ports (e.g., `:8282`) and ensuring all telemetry traffic flows through the authenticated proxy path.
...
- **MediaMTX Control Link**: On Ubuntu, the 'launch-all.sh' script automatically configures UFW to whitelist the Docker subnet for the MediaMTX authenticated control link. For Windows and macOS deployments, the launch scripts (`launch-all.bat` and `launch-all.sh`) automatically download and configure a native `mediamtx` host process to bypass Docker's `network_mode: "host"` container limitations, allowing automated path provisioning for IP cameras to function natively.
- **Graceful Degradation**: The benefits of the MediaMTX proxy (load balancing, stream stability) are fully supported on Linux (via Docker host networking) and Windows/macOS (via automated native host execution).
